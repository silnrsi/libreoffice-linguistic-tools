*******************************************************************************
* LibreOffice Linguistic Tools add-on (LOLT)
* SIL NRSI
*******************************************************************************

LOLT provides a menu of tools for linguistic writeups and other documents
written in lesser-known languages.

For help, requirements and installation instructions, see Linguistics -> Help,
or download the help file in PDF format.


*******************************************************************************
* Version History
*******************************************************************************

To see which version is installed, go to Tools -> Extension manager.

4.1   10-Oct-2024  Interlinear word glosses.
                   Requires LibreOffice 6.1 or higher.
                   Apache OpenOffice is now too far behind.
4.0.2 30-Nov-2021  Updated Data Conversion for Python 3.8 on Windows.
4.0   07-Aug-2021  Improved handling of ref numbers in Flex data.
                   Dialog for oxttools works now; lxml not required.
                   No longer requires a Writer document to store Calc settings.
3.9.2 18-Nov-2019  Update documentation to say LibreOffice.
                   Version 3.9.1 failed to run on linux.
3.9.1 04-Aug-2018  An early version towards 4.0.
                   Data Conversion for Draw - text boxes.
                   Concept dialog for oxttools. Requires python lxml library.
3.1   19-Jan-2018  Select from list of available ref numbers.
3.0   11-Mar-2017  Arrange grammar lines like FieldWorks, not Toolbox.
                   Experimental release of Bulk Conversion.
                   Normalize word lists and spelling.
2.9.1 15-Dec-2015  Option to read Flextext ref numbers.
2.9   30-Nov-2015  Refactor to prepare for 3.0 release.
                   Revise help to focus on FieldWorks.
2.2.1 28-May-2014  Updated for LibreOffice 4.1 and Apache OpenOffice 4.1
                   ECDriver is now included with FieldWorks on Linux.
2.1.2 05-Jul-2013  Choose whether Flex lexeme is phonemic.
2.1.1 10-Jun-2013  Search paths for Linux Data Conversion.
2.1   15-May-2013  Fix Alt hotkeys in dialogs.
                   Improve case insensitive spell checking.
                   Programming: automated test suite.
2.0   25-Apr-2013  Word List and Spelling using Calc spreadsheet.
                   Updated for LibreOffice 4 and Python 3.
                   Minimum Office 3.2 and Python 2.6.
                   Linux ECDriver patch for EncConverters from FieldWorks.
                   Data Conversion can search for complex fonts.
                   Fixed several Spanish and French localization problems.
                   Tested and fixed a number of bugs for grammar and phonology.
1.2.8 16-May-2012  Fixed bug when reading flextext files.
1.2.7 30-Apr-2012  Data Conversion in 1.2.6 on Windows was not recognizing the
                   converter name.
1.2.6 12-Jan-2012  Fix bug when updating examples introduced in version 1.2.5.
1.2.5 22-Dec-2011  Fix problems with Data Conversion introduced in version 1.1.
                   Data Conversion is pure python, no OpenOffice C++ component.
                   Requires Office 3.1 or higher.
1.2.1 28-Oct-2011  Read Source field from Flex LIFT data.
1.2   17-Aug-2011  Script Practice to help learn various scripts.
                   Handle experimental transcriptions from Phonology Assitant.
1.1   23-Apr-2011  Spanish localization and corrected French localization.
                   Data Conversion handles footnotes, character styles and
                   line breaks, and is faster.
1.0.2 13-Nov-2010  Updated French localization.
1.0.1 30-Oct-2010  Fix installation problem on Office 3.3.
1.0   29-Oct-2010  Ability to update examples.
                   Data Conversion can create styles and allows font-to-font.
                   Interlinear orthographic word and morpheme lines.
                   Can specify different tags to read Toolbox data.
                   Import Phonology data from Toolbox as XML, not SFM.
                   Menu uninstaller.
                   Programming: layered design by importing from pythonpath.
0.9.1 25-Aug-2010  Bug fixes for FieldWorks interlinear data.
0.9   05-Aug-2010  Improved Data Conversion: tables in selection, bug fixes.
0.8.3 08-Apr-2010  Figure out optimal number of words for each table row.
                   French localization.
                   Grammar options: free translation in quotes, orthographic line,
                   part of speech above gloss.
                   Abbreviation Add New button improved.
0.8   24-Mar-2010  Improved Data Conversion.
                   Option to not create an outer table for grammar.
                   Fixed significant problem of slow frames.
                   New feature: Abbreviations.
                   Import phonology data directly from Toolbox SFM.
0.7   26-Feb-2010  Extensive testing and bug fixes.
                   Renamed to Linguistic Tools.
                   Experimental version of Data Conversion.
0.6   11-Feb-2010  Handle LIFT data exported from Flex.
                   Removed font and size from Grammar Settings.
0.5   06-Feb-2010  Change interlinear styles to be similar to those of Flex.
                   Tables improved.
0.4   02-Feb-2010  Additional prefixes so that multiple Flex texts can be used.
                   Initial support for tables.
0.3   29-Jan-2010  Help documentation.
0.2   27-Jan-2010  Grammar examples using frames like Flex does.
                   Read XML files exported from Toolbox and Flex.
0.1   19-Jan-2010  First extension for a workshop.
                   Phonology Assistant only.
(0.0) 2009         OOo python macro to import interlinear examples from Toolbox.
                   A single table for each example, wrapping not handled.
                   Used Martin Hosken's sh2xml, no exporting needed.
                   Text file for configuration.

